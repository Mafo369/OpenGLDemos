...
SimpleTriangle::SimpleTriangle(int width, int height, ImVec4 clearColor) : OpenGLDemo(width, height, clearColor) {
    ...
    glm::vec3 d = glm::normalize(nearPlaneTopLeft + (1 + 0.5f) * dXPixel + (1 + .5f) * dYPixel) ;
    glm::vec3 ddx = ((d*d) * dXPixel - (d * dXPixel) * d) / glm::pow(d*d, glm::vec3(1.5f));
    glm::vec3 ddy = ((d*d) * dYPixel - (d * dYPixel) * d) / glm::pow(d*d, glm::vec3(1.5f));

    glm::vec3 d1 = glm::normalize(nearPlaneTopLeft + (1 + 0.5f) * dXPixel + (2 + .5f) * dYPixel) ;


    // Initialise geometric data
    m_color = glm::vec4(1.0f, 0.5f, 0.2f, 1.f);
    Vertex v0 = {normalize(glm::vec3(0.5f,  0.5f, 1.5f)), d, glm::vec2(0.f, 0.f), m_color};
    Vertex v1 = {normalize(glm::vec3(0.5f, -0.5f, 0.0f)), pos, glm::vec2(0.f, 0.f), m_color};
    Vertex v2 = {normalize(glm::vec3(-0.5f, -0.5f, 0.0f)), d1, glm::vec2(0.f, 0.f), glm::vec4(1,0,0,0)};
    ...
    m_programcolor = m_program = 
        new Shader("Shaders/Basic.vert.glsl", "Shaders/Basic.frag.glsl");     
    m_programnormal = 
        new Shader("Shaders/Basic.vert.glsl", "Shaders/Normals.frag.glsl");
}
