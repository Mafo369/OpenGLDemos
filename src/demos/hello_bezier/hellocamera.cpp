...
SimpleCamera::SimpleCamera( int width, int height, ImVec4 clearColor ) :
    ...
    Shader* programNormal  = new Shader( "Shaders/Camera.vert.glsl", "Shaders/Camera.frag.glsl" );
...
void SimpleCamera::compute( bool update ) {
    ...
    RenderObject* ro;
    if ( !m_first ) { ro = new RenderObject( mesh, m_renderer->getCurrentMaterial() ); }
    else {
    ...
    if ( m_displayCtrlPts ) m_renderer->addRenderObject( roCPM );
}
